...
- **Tests must exercise real production code** — not duplicate/reimplement it locally. If tests define their own copy of production logic instead of importing and calling the real function, this is a **Critical** issue. The test will pass even if the production code has a bug.
