...
cat AGENTS.md      # Codebase patterns, architecture, critical conventions
...
- **Tests must exercise real production code** — never duplicate/reimplement production logic locally in tests. If a function can't be imported (e.g., deeply nested closure), extract the testable logic to module level first, then import and test it.
...
**IMPORTANT**: Before committing, check whether `PROGRESS.md` and `PRD.md` are inside the git repo or outside it. If they are outside the git working tree (e.g., workspace root vs. a subfolder repo), do NOT attempt to `git add` them — they will fail silently. Only add files that are within the repo boundary.
