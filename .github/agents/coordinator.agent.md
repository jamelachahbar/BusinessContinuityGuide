...
1. **Read State**
   ...
   - Read `AGENTS.md` for codebase patterns, architecture, and critical conventions
...
3. **Spawn Executor Subagent**
   ...
   - Include relevant context from `AGENTS.md` (critical patterns, common issues, file locations)
...
## Ad-Hoc Bugs (Post-PRD)

When a production bug is reported mid-session (not in the original PRD):

1. Investigate the root cause yourself (read logs, traces, code)
2. Add a new task to `PRD.md` (e.g., Task-008) with full context, implementation plan, and acceptance criteria
3. Update `PROGRESS.md` with the new iteration
4. Spawn Executor → Reviewer as normal
5. After completion, save learnings to repository memory
